...
async function createOrder(goods) {
  // let method = 'alipay.trade.page.pay'; // PC端统一收单下单并支付页面接口
  let method = "alipay.trade.wap.pay"; // 移动端统一收单下单并支付页面接口
  ...
  let bizContent = {
    ...
    // product_code: 'FAST_INSTANT_TRADE_PAY', // PC端商品码，当前只支持这个
    product_code: "QUICK_WAP_WAY", // 移动端商品码，当前只支持这个
  ...
  // formData.addField('returnUrl', 'http://27.38.28.222:9999/payresult'); // 客户端支付成功后会同步跳回的地址
  // formData.addField('notifyUrl', 'http://27.38.28.222:9999/notify.html'); // 支付宝在用户支付成功后会异步通知的回调地址，必须在公网 IP 上才能收到
  formData.addField("bizContent", bizContent); // 将必要的参数集合添加进 form 表单

  // 异步向支付宝发送生成订单请求，第二个参数为公共参数，不需要的话传入空对象就行
  const result = await alipay.exec(method, {}, { formData: formData });
  // 返回订单的结果信息
  return result;
}
